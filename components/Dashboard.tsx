...
const UserProfileCard: React.FC<{ userProfile: UserProfile; currentXp: number; totalXp: number; onReset: () => void; }> = ({ userProfile, currentXp, totalXp, onReset }) => {
...
const TaskItem: React.FC<{ task: LearningTask; onToggle: () => void; onPriorityToggle: () => void; }> = ({ task, onToggle, onPriorityToggle }) => {
    return (
        <div className={`flex items-start gap-4 p-4 rounded-lg transition-all duration-300 ${task.completed ? 'bg-green-500/10' : 'bg-gray-800'} ${task.priority ? 'ring-2 ring-red-500' : ''}`}>
            ...
            <div className="flex-1">
                <h4 className={`font-semibold ${task.completed ? 'line-through text-gray-400' : 'text-white'}`}>{task.title}</h4>
...
const ModuleCard: React.FC<{ module: LearningModule; onTaskToggle: (taskId: string) => void; onPriorityToggle: (taskId: string) => void; }> = ({ module, onTaskToggle, onPriorityToggle }) => {
    ...
    return (
        <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-2xl p-6 shadow-lg animate-slide-in" style={{ animationDelay: '200ms' }}>
            ...
            <div className="space-y-3">
                {module.tasks.map(task => <TaskItem key={task.id} task={task} onToggle={() => onTaskToggle(task.id)} onPriorityToggle={() => onPriorityToggle(task.id)} />)}
            </div>
        </div>
    );
};
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <aside className="lg:col-span-1 lg:sticky top-8 self-start">
            <UserProfileCard userProfile={userProfile} currentXp={currentXp} totalXp={totalXp} onReset={handleReset} />
        </aside>
